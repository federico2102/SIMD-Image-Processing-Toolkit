...
.ciclo:
	...
	pmaxsw xmm0, xmm1 ; xmm0 = [Pixel 1 = max(A,R) | max(R,G) | max(G,B) | max(B,0), Pixel 2 = max(A,R) | max(R,G) | max(G,B) | max(B,0)]
	psllq xmm1, 16 ;xmm1 = [Pixel 1 = G | B | 0 | 0, Pixel 2 = G | B | 0 | 0]
	pmaxsw xmm0, xmm1 ; xmm0 = [Pixel 1 = max(A,R,G) | max(R,G,B) | max(G,B,0) | max(B,0,0), Pixel 2 = max(A,R,G) | max(R,G,B) | max(G,B,0) | max(B,0,0)]
	
	; CONSULTAR LA PARTE DEL SHUFFLE

	psllq xmm0, 16 ;xmm1 = [Pixel 1 = max(R,G,B) | max(G,B, 0) | max(B, 0, 0) | 0, Pixel 2 = max(R,G,B) | max(G,B, 0) | max(B, 0, 0) | 0]
	psrlq xmm0, 48 ;xmm1 = [Pixel 1 = 0 | 0 | 0 | max(R,G,B), Pixel 2 = 0 | 0 | 0 |  max(R,G,B)]
	pxor xmm1, xmm1
	paddusw xmm1, xmm0
	psllq xmm0, 16
	paddusw xmm1, xmm0
	...
	pmaxsw xmm2, xmm3 ; xmm2 = [Pixel 1 = max(A,R) | max(R,G) | max(G,B) | max(B,0), Pixel 2 = max(A,R) | max(R,G) | max(G,B) | max(B,0)]
	psllq xmm3, 16 ;xmm3 = [Pixel 1 = G | B | 0 | 0, Pixel 2 = G | B | 0 | 0]
	pmaxsw xmm2, xmm3 ; xmm2 = [Pixel 1 = max(A,R,G) | max(R,G,B) | max(G,B,0) | max(B,0,0), Pixel 2 = max(A,R,G) | max(R,G,B) | max(G,B,0) | max(B,0,0)]

	psllq xmm2, 16 ;xmm1 = [Pixel 1 = max(R,G,B) | max(G,B, 0) | max(B, 0, 0) | 0, Pixel 2 = max(R,G,B) | max(G,B, 0) | max(B, 0, 0) | 0]
	psrlq xmm2, 48 ;xmm1 = [Pixel 1 = 0 | 0 | 0 | max(R,G,B), Pixel 2 = 0 | 0 | 0 |  max(R,G,B)]
	pxor xmm3, xmm3
	paddusw xmm3, xmm2
	psllq xmm2, 16
	paddusw xmm3, xmm2
	...
	; NUEVO
	; pand xmm1, xmm11 ;xmm8 = [Pixel 1 = 0 | max(R,G,B) | max(R,G,B) | max(R,G,B), Pixel 2 = 0 | max(R,G,B) | max(R,G,B) | max(R,G,B), Pixel 3 = 0 | max(R,G,B) | max(R,G,B) | max(R,G,B), Pixel 4 = 0 | max(R,G,B) | max(R,G,B) | max(R,G,B) ]
	por xmm1, xmm8 ;xmm7 = [Pixel 1 = A | max(R,G,B) | max(R,G,B) | max(R,G,B), Pixel 2 = A | max(R,G,B) | max(R,G,B) | max(R,G,B), Pixel 3 = A | max(R,G,B) | max(R,G,B) | max(R,G,B), Pixel 4 = A | max(R,G,B) | max(R,G,B) | max(R,G,B) ]
